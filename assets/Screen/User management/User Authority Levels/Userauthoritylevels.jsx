...
export default function Userauthoritylevels() {
    const [value, setvalue] = useState({
        UserAuthorityCodeserach: '', WorkRequest: '', UserAuthorityCode: '', UserAuthoritySeq: '', UserAuthorityDesc:'',
    ...
    const toggleDialog2 = (UserAuthorityCode) => {
        setDeleteItemCode(UserAuthorityCode); // Store the system module code
    ...
    const toggleshowmodel = (UserAuthorityCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(UserAuthorityCode)
        axios.get(`/api/UserAuthority_GET_BYID/${UserAuthorityCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    UserAuthoritySeq: res.data.recordset[0].UserAuthoritySeq,
                    UserAuthorityCode: res.data.recordset[0].UserAuthorityCode,
                    UserAuthorityDesc: res.data.recordset[0].UserAuthorityDesc
                }))
            })
            .catch((err) => {
                console.log(err);
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/UserAuthority_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>User Authority Levels Maintenance
                    </Text>
                    <View style={styles.inputContainer}>
                        ...
                        <View style={{ marginTop: 10 }}>
                            ...
                            <TextInput
                                ...
                                value={value.UserAuthorityCodeserach}
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        UserAuthorityCodeserach: item,
                                    }));
                                }}
                                placeholder="Select User Authority Code"
                ...
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>USER LEVELS CODE </Text></DataTable.Title>
                            <DataTable.Title style={[styles.header, { width: 200 }]} ><Text style={styles.tableHeading}>USER AUTHORITY SEQ </Text></DataTable.Title>
                        ...
                        {items.filter(
                            ...
                            <DataTable.Row key={item.UserAuthorityCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        status={item.selected ? 'checked' : 'unchecked'}
                                        onPress={() => handleCheckboxChange(item.UserAuthorityCode)}
                                    />
                                </DataTable.Cell>
                                ...
                                <DataTable.Cell style={[styles.tablebody, { width: 200 }]}>{item.UserAuthoritySeq}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.UserAuthorityDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.UserAuthorityCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.UserAuthorityCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Adduserauthority myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={showmodel} >
                    <View style={{ marginTop: 20 }}>
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>User Authorit Description
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                ...
                                placeholder="User Authorit Description"
                                placeholderTextColor="#94A0CA"
                                selectionColor="#1D3A9F"
                                underlineColorAndroid="transparent"
                            />
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            ...
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                value={value.UserAuthoritySeq.toString()}
                                onChangeText={text => {
                                    if (/^\d+$/.test(text) || text === "") {
                                        setvalue(prevValue => ({
                                            ...prevValue,
