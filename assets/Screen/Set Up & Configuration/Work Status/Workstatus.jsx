...
import Crateworkstatus from '../../../Component/Setup and configuration/WORK STATUS/Crateworkstatus';

export default function Workstatus() {
    const [value, setvalue] = useState({
        WorkStatusCodeserach: '', WorkRequest: '', WorkStatusCode: '', WorkStatusDesc: ''
    ...
    const handleCheckboxChange = (WorkStatusCode) => {
        const updatedItems = items.map((item) =>
            item.WorkStatusCode === WorkStatusCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.WorkStatusCode);
    ...
    const toggleDialog2 = (WorkStatusCode) => {
        setDeleteItemCode(WorkStatusCode); // Store the system module code
    ...
    const toggleshowmodel = (WorkStatusCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(WorkStatusCode)
        axios.get(`/api/WorkStatus_GET_BYID/${WorkStatusCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    WorkStatusDesc: res.data.recordset[0].WorkStatusDesc,
                    WorkStatusCode: res.data.recordset[0].WorkStatusCode,
                }))
            })
            .catch((err) => {
                console.log(err);
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/WorkStatus_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>WORK STATUS MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal vertical>
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>WORK STATUS CODE</Text></DataTable.Title>
                        ...
                        <ScrollView>
                            {items.slice(from, to).map((item, index) => (
                                <DataTable.Row key={item.WorkStatusCode}>
                                    <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                        <Checkbox
                                            status={item.selected ? 'checked' : 'unchecked'}
                                            onPress={() => handleCheckboxChange(item.WorkStatusCode)}
                                        />
                                    </DataTable.Cell>
                                    <DataTable.Cell style={[styles.tablebody, { width: 80 }]}>{index + 1}</DataTable.Cell>
                                    <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.WorkStatusCode}</DataTable.Cell>
                                    <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.WorkStatusDesc}</DataTable.Cell>
                                    <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                        <View>
                                            <View style={styles.actions}>
                                                <TouchableOpacity onPress={() => toggleshowmodel(item.WorkStatusCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                    <FontAwesome5 name="sync-alt" size={20} color="black" />
                                                </TouchableOpacity>
                                                <TouchableOpacity onPress={() => toggleDialog2(item.WorkStatusCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Crateworkstatus myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} work Status`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Work Trade" />

                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            <Text style={styles.lableinput}>Work Status Code
                            </Text>
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        WorkStatusCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                                placeholder="Work Status Code"
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            ...
                            <TextInput
                                ...
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        WorkStatusDesc: item, // Update the Employeeid property
                                    }));
                                }}
                                placeholder="Work Status Code"
                                placeholderTextColor="#94A0CA"
