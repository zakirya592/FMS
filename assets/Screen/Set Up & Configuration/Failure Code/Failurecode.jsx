...
import Createfailurecode from '../../../Component/Setup and configuration/Failure Code/Createfailurecode';

export default function Failurecode() {
    const [value, setvalue] = useState({
        FailureStatusCodeserach: '', WorkRequest: '', FailureStatusCode: '', FailureStatusDesc: ''
    ...
    const handleCheckboxChange = (FailureStatusCode) => {
        const updatedItems = items.map((item) =>
            item.FailureStatusCode === FailureStatusCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.FailureStatusCode);
    ...
    const toggleDialog2 = (FailureStatusCode) => {
        setDeleteItemCode(FailureStatusCode); // Store the system module code
    ...
    const toggleshowmodel = (FailureStatusCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(FailureStatusCode)
        axios.get(`/api/Failure_GET_BYID/${FailureStatusCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    FailureStatusDesc: res.data.recordset[0].FailureStatusDesc,
                    FailureStatusCode: res.data.recordset[0].FailureStatusCode,
                }))
            }).catch((err) => {
                console.log(err);
            });
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/Failure_Put/${updataItemCode}`, {
            FailureStatusDesc: value.FailureStatusDesc,
        },).then((res) => {
            getapi()
            setshowmodel(!showmodel);
        }).catch((err) => {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>FAILURE CODE MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal vertical>
                    <DataTable style={[styles.item, {
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.FailureStatusCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        status={item.selected ? 'checked' : 'unchecked'}
                                        onPress={() => handleCheckboxChange(item.FailureStatusCode)}
                                    />
                                </DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 80 }]}>{index + 1}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.FailureStatusCode}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.FailureStatusDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.FailureStatusCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.FailureStatusCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Createfailurecode myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} FAILURE`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Failure" />
                    ...
                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            ...
                            <TextInput
                                ...
                                value={value.FailureStatusCode}
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        FailureStatusCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Failure Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                value={value.FailureStatusDesc}
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        FailureStatusDesc: item, // Update the Employeeid property
                                    }));
                                }}
                                placeholder="Work Failure Desc"
                                placeholderTextColor="#94A0CA"
                                selectionColor="#1D3A9F"
                                underlineColorAndroid="transparent"
                            />
