...
import CreateTitlesatuation from '../../../Component/Setup and configuration/Title Saluation/CreateTitlesatuation';

export default function Titlesatutation() {
    const [value, setvalue] = useState({
        TitleCodeserach: '', WorkRequest: '', TitleCode: '', TitleDesc: ''
    ...
    const handleCheckboxChange = (TitleCode) => {
        const updatedItems = items.map((item) =>
            item.TitleCode === TitleCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.TitleCode);
    ...
    const toggleDialog2 = (TitleCode) => {
        setDeleteItemCode(TitleCode); // Store the system module code
    ...
    const toggleshowmodel = (TitleCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(TitleCode)
        axios.get(`/api/Title_GET_BYID/${TitleCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    TitleDesc: res.data.recordset[0].TitleDesc,
                    TitleCode: res.data.recordset[0].TitleCode,
                }))
            }).catch((err) => {
                console.log(err);
            });
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/Title_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>Title /Saluation Maintenance
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>TITLE CODE</Text></DataTable.Title>
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.TitleCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        status={item.selected ? 'checked' : 'unchecked'}
                                        onPress={() => handleCheckboxChange(item.TitleCode)}
                                    />
                                </DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 80 }]}>{index + 1}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.TitleCode}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.TitleDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.TitleCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.TitleCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <CreateTitlesatuation myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} Title`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Title /Saluation Maintenance" />
                    ...
                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            ...
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        TitleCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Title Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
