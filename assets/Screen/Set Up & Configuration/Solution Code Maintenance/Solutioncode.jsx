...
import Createsolutioncode from '../../../Component/Setup and configuration/Solution Code/Createsolutioncode';

export default function Solutioncode() {
    const [value, setvalue] = useState({
        SolutiontatusCodeserach: '', WorkRequest: '', SolutiontatusCode: '', SolutionStatusDesc: ''
    ...
    const handleCheckboxChange = (SolutiontatusCode) => {
        const updatedItems = items.map((item) =>
            item.SolutiontatusCode === SolutiontatusCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.SolutiontatusCode);
    ...
    const toggleDialog2 = (SolutiontatusCode) => {
        setDeleteItemCode(SolutiontatusCode); // Store the system module code
    ...
    const toggleshowmodel = (SolutiontatusCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(SolutiontatusCode)
        axios.get(`/api/Solution_GET_BYID/${SolutiontatusCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    SolutionStatusDesc: res.data.recordset[0].SolutionStatusDesc,
                    SolutiontatusCode: res.data.recordset[0].SolutiontatusCode,
                }))
            }).catch((err) => {
                console.log(err);
            });
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/Solution_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>SOLUTION CODE MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>SOLUTION CODE</Text></DataTable.Title>
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.SolutiontatusCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        status={item.selected ? 'checked' : 'unchecked'}
                                        onPress={() => handleCheckboxChange(item.SolutiontatusCode)}
                                    />
                                </DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 80 }]}>{index + 1}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.SolutiontatusCode}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.SolutionStatusDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.SolutiontatusCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.SolutiontatusCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Createsolutioncode myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} Solution Status`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Solution Status" />

                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            <Text style={styles.lableinput}>Solution Status Code
                            </Text>
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        SolutiontatusCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                                placeholder="Solution Status Code"
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Solution Status  Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                ...
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        SolutionStatusDesc: item, // Update the Employeeid property
                                    }));
                                }}
                                placeholder="Solution Status Desc"
                                placeholderTextColor="#94A0CA"
                                selectionColor="#1D3A9F"
                                underlineColorAndroid="transparent"
                            />
