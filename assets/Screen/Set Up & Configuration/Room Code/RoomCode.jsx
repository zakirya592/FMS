...
import Createroomcode from '../../../Component/Setup and configuration/Room Code/Createroomcode';

export default function RoomCode() {
    const [value, setvalue] = useState({
        RoomCodeserach: '', WorkRequest: '', RoomCode: '', RoomDesc: ''
    ...
    const toggleDialog2 = (RoomCode) => {
        setDeleteItemCode(RoomCode); // Store the system module code
    ...
    const toggleshowmodel = (RoomCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(RoomCode)
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/Rooms_Put/${updataItemCode}`, {
        ...
        <ScrollView>
            <View>
                ...
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>ROOM CODE</Text></DataTable.Title>
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.RoomCode}>
                                ...
                                <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.RoomCode}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.RoomDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.RoomCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.RoomCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Createroomcode myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} Room`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Room" />

                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            <Text style={styles.lableinput}>Room Code
                            </Text>
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        RoomCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Room Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                ...
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        RoomDesc: item, // Update the Employeeid property
                                    }));
                                }}
