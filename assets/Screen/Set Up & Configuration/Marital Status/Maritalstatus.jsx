...
import Createcarital from '../../../Component/Setup and configuration/Marital Status/Createcarital';

export default function Maritalstatus() {
    const [value, setvalue] = useState({
        MaritalCodeserach: '', WorkRequest: '', MaritalCode: '', MaritalDesc: ''
    })
    ...
    const handleCheckboxChange = (MaritalCode) => {
        const updatedItems = items.map((item) =>
            item.MaritalCode === MaritalCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.MaritalCode);
    ...
    const toggleDialog2 = (MaritalCode) => {
        setDeleteItemCode(MaritalCode); // Store the system module code
        setVisible2(!visible2);
    };

    const Deletedapi = (MaritalCode) => {
        axios.delete(`/api/MaritalStatus_DELETE_BYID/${MaritalCode}`)
            .then((res) => {
                setVisible2(false);
                getapi()
            })
    ...
    const toggleshowmodel = (MaritalCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(MaritalCode)
        axios.get(`/api/MaritalStatus_GET_BYID/${MaritalCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    MaritalDesc: res.data.recordset[0].MaritalDesc,
                    MaritalCode: res.data.recordset[0].MaritalCode,
                }))
            }).catch((err) => {
                console.log(err);
            });
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/MaritalStatus_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>MARITAL MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>MARITAL STATUS CODE </Text></DataTable.Title>
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.MaritalCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        status={item.selected ? 'checked' : 'unchecked'}
                                        onPress={() => handleCheckboxChange(item.MaritalCode)}
                                    />
                                </DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 80 }]}>{index + 1}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.MaritalCode}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.MaritalDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.MaritalCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.MaritalCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Createcarital myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} Marital`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Marital" />

                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            <Text style={styles.lableinput}>Marital Code
                            </Text>
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        MaritalCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Marital Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                ...
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        MaritalDesc: item, // Update the Employeeid property
                                    }));
                                }}
