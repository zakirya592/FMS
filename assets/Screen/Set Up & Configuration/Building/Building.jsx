...
import Createbuilding from '../../../Component/Setup and configuration/Building/Createbuilding';

export default function Building() {
    const [value, setvalue] = useState({
        BuildingCodeserach: '', WorkRequest: '', BuildingCode: '', BuildingDesc: ''
    ...
    const toggleshowmodel = (BuildingCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(BuildingCode)
        axios.get(`/api/Building_GET_BYID/${BuildingCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    BuildingDesc: res.data.recordset[0].BuildingDesc,
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/Building_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>WORK BUILDING MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                    ...
                    <DataTable style={[styles.item, {
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.BuildingCode}>
                                ...
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.BuildingDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.BuildingCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.BuildingCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Createbuilding myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} Building`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Building" />

                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            <Text style={styles.lableinput}>Work Building Code
                            </Text>
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        BuildingCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                                placeholder="Work Building Code"
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Work Building Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                ...
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        BuildingDesc: item, // Update the Employeeid property
                                    }));
                                }}
                                placeholder="Work Building Desc"
                                placeholderTextColor="#94A0CA"
                                selectionColor="#1D3A9F"
                                underlineColorAndroid="transparent"
                            />
