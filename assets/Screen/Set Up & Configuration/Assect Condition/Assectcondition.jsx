...
import Createcssetcondition from '../../../Component/Setup and configuration/Asset Condition/Createcssetcondition';

export default function Assectcondition() {
    const [value, setvalue] = useState({
        AssetConditionCodeserach: '', WorkRequest: '', AssetConditionCode: '', AssetConditionDesc: ''
    ...
    const handleCheckboxChange = (AssetConditionCode) => {
        const updatedItems = items.map((item) =>
            item.AssetConditionCode === AssetConditionCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.AssetConditionCode);
    ...
    const toggleDialog2 = (AssetConditionCode) => {
        setDeleteItemCode(AssetConditionCode); // Store the system module code
    ...
    const toggleshowmodel = (AssetConditionCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(AssetConditionCode)
        axios.get(`/api/AssetCondition_GET_BYID/${AssetConditionCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    AssetConditionDesc: res.data.recordset[0].AssetConditionDesc,
                    AssetConditionCode: res.data.recordset[0].AssetConditionCode,
                }))
            }).catch((err) => {
                console.log(err);
            });
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/AssetCondition_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>ASSET CONDITION MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>ASSET CONDITION CODE</Text></DataTable.Title>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Createcssetcondition myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} Asset Condition`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Asset Condition" />

                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            <Text style={styles.lableinput}>Asset Condition Code
                            </Text>
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        AssetConditionCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                                placeholder="Asset Condition Code"
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Asset Condition Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                ...
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        AssetConditionDesc: item, // Update the Employeeid property
                                    }));
                                }}
                                placeholder="Asset Condition Desc"
                                placeholderTextColor="#94A0CA"
                                selectionColor="#1D3A9F"
                                underlineColorAndroid="transparent"
                            />
