...
import Createwarrantyperiod from '../../../Component/Setup and configuration/Warranty Period/Createwarrantyperiod';
...
export default function WarrantyPeriod() {
    ...
    const handleCheckboxChange = (WarrantyPeriodCode) => {
        const updatedItems = items.map((item) =>
            item.WarrantyPeriodCode === WarrantyPeriodCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.WarrantyPeriodCode);
    ...
    const toggleDialog2 = (WarrantyPeriodCode) => {
        setDeleteItemCode(WarrantyPeriodCode); // Store the system module code
    ...
    const toggleshowmodel = (WarrantyPeriodCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(WarrantyPeriodCode)
        axios.get(`/api/WarrantyPeriod_GET_BYID/${WarrantyPeriodCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    WarrantyPeriodDesc: res.data.recordset[0].WarrantyPeriodDesc,
                }))
            }).catch((err) => {
                console.log(err);
            });
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/WarrantyPeriod_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>WARRANTY PERIOD MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 200 }]} ><Text style={styles.tableHeading}>WARRANTY STATUS CODE</Text></DataTable.Title>
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.WarrantyPeriodCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        status={item.selected ? 'checked' : 'unchecked'}
                                        onPress={() => handleCheckboxChange(item.WarrantyPeriodCode)}
                                    />
                                </DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 80 }]}>{index + 1}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 200 }]}>{item.WarrantyPeriodCode}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.WarrantyPeriodDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.WarrantyPeriodCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.WarrantyPeriodCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Createwarrantyperiod myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} Warranty Period`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Warranty Period" />
                    ...
                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            ...
                            <TextInput
                                ...
                                value={updataItemCode}
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        WarrantyPeriodCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Warranty Period Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                ...
                                placeholder="Warranty Period Desc"
                                placeholderTextColor="#94A0CA"
                                selectionColor="#1D3A9F"
                                underlineColorAndroid="transparent"
                            />
