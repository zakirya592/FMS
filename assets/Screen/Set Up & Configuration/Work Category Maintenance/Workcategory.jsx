...
import Createworkcatagres from '../../../Component/Setup and configuration/Work Category/Createworkcatagres';

export default function Workcategory() {
    const [value, setvalue] = useState({
        WorkCategoryCodeserach: '', WorkRequest: '', WorkCategoryCode: '', WorkCategoryDesc: ''
    ...
    const handleCheckboxChange = (WorkCategoryCode) => {
        const updatedItems = items.map((item) =>
            item.WorkCategoryCode === WorkCategoryCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.WorkCategoryCode);
    ...
    const toggleDialog2 = (WorkCategoryCode) => {
        setDeleteItemCode(WorkCategoryCode); // Store the system module code
    ...
    const toggleshowmodel = (WorkCategoryCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(WorkCategoryCode)
        axios.get(`/api/WorkCatagres_GET_BYID/${WorkCategoryCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    WorkCategoryDesc: res.data.recordset[0].WorkCategoryDesc,
                    WorkCategoryCode: res.data.recordset[0].WorkCategoryCode,
                }))
            })
            .catch((err) => {
                console.log(err);
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/WorkCatagres_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>WORK Category MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal vertical>
                    <DataTable style={[styles.item, {
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.WorkCategoryCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        status={item.selected ? 'checked' : 'unchecked'}
                                        onPress={() => handleCheckboxChange(item.WorkCategoryCode)}
                                    />
                                </DataTable.Cell>
                                ...
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.WorkCategoryDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.WorkCategoryCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.WorkCategoryCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Createworkcatagres myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} work Category`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Work Category" />
                        ...
                        <View style={styles.singleinputlable}>
                            ...
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        WorkCategoryCode: item.value, // Update the Employeeid property
                                    }));
                                }}
