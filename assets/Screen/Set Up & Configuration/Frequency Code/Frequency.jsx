...
import Createfrequency from '../../../Component/Setup and configuration/Frequency/Createfrequency';

export default function Frequency() {
    const [value, setvalue] = useState({
        FreqCodeserach: '', FreqSeq: '', FreqCode: '', FreqDesc: ''
    ...
    const handleCheckboxChange = (FreqCode) => {
        const updatedItems = items.map((item) =>
            item.FreqCode === FreqCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.FreqCode);
    ...
    const toggleDialog2 = (FreqCode) => {
        setDeleteItemCode(FreqCode); // Store the system module code
    ...
    const toggleshowmodel = (FreqCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(FreqCode)
        axios.get(`/api/Frequency_GET_BYID/${FreqCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    FreqDesc: res.data.recordset[0].FreqDesc,
                    FreqCode: res.data.recordset[0].FreqCode,
                    FreqSeq: res.data.recordset[0].FreqSeq,
                }))
            }).catch((err) => {
                console.log(err);
            });
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/Frequency_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>Frequency MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>FREQENCY CODE</Text></DataTable.Title>
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>FREQENCY SEQ</Text></DataTable.Title>
                            ...
                            <DataTable.Row key={item.FreqCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        ...
                                        onPress={() => handleCheckboxChange(item.FreqCode)}
                                    />
                                </DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 80 }]}>{index + 1}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.FreqCode}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.FreqSeq}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.FreqDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.FreqCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.FreqCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Createfrequency myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} Frequency`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Frequency " />

                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            <Text style={styles.lableinput}>Frequency Code
                            </Text>
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        FreqCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Frequency Seq
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                value={value.FreqSeq.toString()}
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Frequency Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
