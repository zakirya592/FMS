...
import Creategender from '../../../Component/Setup and configuration/Gender Code/Creategender';

export default function Gendercode() {
    const [value, setvalue] = useState({
        GenderCodeserach: '', WorkRequest: '', GenderCode: '', GenderDesc: ''
    ...
    const handleCheckboxChange = (GenderCode) => {
        const updatedItems = items.map((item) =>
            item.GenderCode === GenderCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.GenderCode);
    ...
    const toggleDialog2 = (GenderCode) => {
        setDeleteItemCode(GenderCode); // Store the system module code
        setVisible2(!visible2);
    };

    const Deletedapi = (GenderCode) => {
        axios.delete(`/api/Gender_DELETE_BYID/${GenderCode}`)
            .then((res) => {
                setVisible2(false);
                getapi()
            })
    ...
    const toggleshowmodel = (GenderCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(GenderCode)
        axios.get(`/api/Gender_GET_BYID/${GenderCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    GenderDesc: res.data.recordset[0].GenderDesc,
                    GenderCode: res.data.recordset[0].GenderCode,
                }))
            }).catch((err) => {
                console.log(err);
            });
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/Gender_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>GENDER MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        <DataTable.Header>
                            ...
                            <DataTable.Title style={[styles.header, { width: 170 }]} ><Text style={styles.tableHeading}>GENDER CODE</Text></DataTable.Title>
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.GenderCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        status={item.selected ? 'checked' : 'unchecked'}
                                        onPress={() => handleCheckboxChange(item.GenderCode)}
                                    />
                                </DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 80 }]}>{index + 1}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.GenderCode}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.GenderDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.GenderCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.GenderCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <Creategender myFunction={getapi} />
                </View>
                ...
                <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
                    ...
                    <Text>{`You want to delete this ${deleteItemCode} Gender`}</Text>
                    <Dialog.Actions >
                ...
                <Dialog isVisible={showmodel} >
                    <Dialog.Title title="Updata Gender" />

                    <View style={{ marginTop: 20 }}>
                        <View style={styles.singleinputlable}>
                            <Text style={styles.lableinput}>Gender Code
                            </Text>
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        GenderCode: item.value, // Update the Employeeid property
                                    }));
                                }}
                        ...
                        <View style={[styles.singleinputlable, { marginTop: 15 }]}>
                            <Text style={styles.lableinput}>Gender Desc
                            </Text>
                            <TextInput
                                style={[styles.inputBox, { width: 240 }]}
                                ...
                                onChangeText={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        GenderDesc: item, // Update the Employeeid property
                                    }));
                                }}
