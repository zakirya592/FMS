...
import CreateAssettype from '../../../Component/Setup and configuration/Asset Type/CreateAssettype';

export default function Assettype() {
    const [value, setvalue] = useState({
        AssetTypeCodeserach: '', WorkRequest: '', AssetTypeCode: '', AssetTypeDesc: ''
    ...
    const getapi = () => {
        axios.get(`/api/AssetType_GET_LIST`)
            .then((res) => {
    ...
    const handleCheckboxChange = (AssetTypeCode) => {
        const updatedItems = items.map((item) =>
            item.AssetTypeCode === AssetTypeCode ? { ...item, selected: !item.selected } : item
        );
        setItems(updatedItems);
        // Update selectedItems state
        const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.AssetTypeCode);
    ...
    const toggleDialog2 = (AssetTypeCode) => {
        setDeleteItemCode(AssetTypeCode); // Store the system module code
        setVisible2(!visible2);
    };

    const Deletedapi = (AssetTypeCode) => {
        axios.delete(`/api/AssetType_DELETE_BYID/${AssetTypeCode}`)
            .then((res) => {
                setVisible2(false);
                getapi()
            })
    ...
    const toggleshowmodel = (AssetTypeCode) => {
        setshowmodel(!showmodel);
        setupdataItemCode(AssetTypeCode)
        axios.get(`/api/AssetType_GET_BYID/${AssetTypeCode}`)
            .then((res) => {
                setvalue((prevValue) => ({
                    ...prevValue,
                    AssetTypeDesc: res.data.recordset[0].AssetTypeDesc,
                    AssetTypeCode: res.data.recordset[0].AssetTypeCode,
                }))
            }).catch((err) => {
                console.log(err);
            });
    ...
    const postapi = (updataItemCode) => {
        axios.put(`/api/AssetType_Put/${updataItemCode}`, {
    ...
    return (
        <ScrollView>
            <View>
                {/* Top section */}
                <View >
                    <Text style={styles.prograp}>ASSET TYPE MAINTENANCE
                    </Text>
                </View>
                {/* table section */}
                <ScrollView horizontal >
                    <DataTable style={[styles.item, {
                        ...
                        {items.slice(from, to).map((item, index) => (
                            <DataTable.Row key={item.AssetTypeCode}>
                                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                                    <Checkbox
                                        status={item.selected ? 'checked' : 'unchecked'}
                                        onPress={() => handleCheckboxChange(item.AssetTypeCode)}
                                    />
                                </DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 80 }]}>{index + 1}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 170 }]}>{item.AssetTypeCode}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 250 }]}>{item.AssetTypeDesc}</DataTable.Cell>
                                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}>
                                    <View>
                                        <View style={styles.actions}>
                                            <TouchableOpacity onPress={() => toggleshowmodel(item.AssetTypeCode)} style={[styles.actions, { marginRight: 10 }]}>
                                                <FontAwesome5 name="sync-alt" size={20} color="black" />
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => toggleDialog2(item.AssetTypeCode)} style={styles.actions}>
                ...
                <View style={[styles.buttonsection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]} >
                    <CreateAssettype myFunction={getapi} />
                </View>
                        ...
                        <View style={styles.singleinputlable}>
                            ...
                            <TextInput
                                ...
                                onChange={item => {
                                    setvalue((prevValue) => ({
                                        ...prevValue,
                                        AssetTypeCode: item.value, // Update the Employeeid property
                                    }));
                                }}
