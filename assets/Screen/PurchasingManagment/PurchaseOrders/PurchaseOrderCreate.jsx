...
export default function PurchaseOrderCreate({ route }) {
  ...
  const Requestnumberapi = () => {
    axios.get(`/api/workRequestCount_GET_BYID/1`)
      .then((res) => {
        ...
        setvalue(prevState => ({ ...prevState, PurchaseOrderNumber: formattedRequestNumber }));
      })
      .catch((err) => {
        console.log(err);
      });
    ...
    axios.get(`/api/workRequestCount_GET_BYID/1`)
      .then((res) => {
        ...
        axios.put(`/api/PurchaseOrderNumber_Put/1`, {
          ...
          .then((res) => {
            const reqput = res.data.recordset[0].PurchaseOrderNumber + 1;
            setvalue(prevState => ({ ...prevState, PurchaseOrderNumber: '000-000-' + '0' + `${reqput}` }));
          })
          .catch((err) => {
            console.log(err);
          });
  ...
  return (
    ...
    <ScrollView contentContainerStyle={styles.containerscrollview}>
      <View>
        <View >
          <Text style={styles.prograp}>Create Purchase Orders</Text>
        </View>
        {/* Purchase Request  */}
        ...
        <AwesomeAlert
          ...
          message={` Purachase Order ${value.PurchaseOrderNumber} has been created successfully`}
          confirmButtonColor="#DD6B55"
          confirmButtonStyle={{ backgroundColor: 'black' }}
