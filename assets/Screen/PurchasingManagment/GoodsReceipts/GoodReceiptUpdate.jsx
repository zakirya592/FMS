...
export default function GoodReceiptUpdate({ route }) {
  ...
  const getapdata = () => {
    axios.get(`/api/GoodsReceipt_GET_BYID/${PurchaseOrderNumber}`)
      .then((res) => {
        setvalue((prevValue) => ({
          ...prevValue,
        ...
        const InvoiceDateget = res.data.recordset[0].InvoiceDate
        const DeliveryDateeever = res.data.recordset[0].ActualDeliveryDate

        setOverallTotalPricess(res.data.recordset[0].TOTAL_AMOUNT)
        setDate(new Date(InvoiceDateget))
        setActualDeliveryDate(new Date(DeliveryDateeever))
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const Createapi = () => {
    axios.put(`/api/GoodsReceipt_Put/${PurchaseOrderNumber}`, {
  ...
  return (
    ...
    <ScrollView contentContainerStyle={styles.containerscrollview}>
      <View>
        <View >
          <Text style={styles.prograp}>Modify Goods Receipts</Text>
        </View>
        {/* Purchase Order  */}
        ...
        <AwesomeAlert
          ...
          message={`Goods Receipts ${PurchaseOrderNumber} has been Updated `}
          confirmButtonColor="#DD6B55"
          confirmButtonStyle={{ backgroundColor: 'black' }}
