...
export default function GoodReceiptable() {
  ...
  const updatbutton = () => {
    if (selectedItems.length >= 1) {
      navigation.navigate(`GoodReceiptUpdate`, { PurchaseOrderNumber: selectedItems, myFunction: getapi })
    }
    else {
      ...
      <View>
        ...
        <ScrollView horizontal vertical>
          <DataTable style={[styles.item, {
            ...
            <DataTable.Header>
              ...
              <DataTable.Title style={[styles.header, { width: 150 }]} ><Text style={styles.tableHeading}>Discount Amount</Text></DataTable.Title>
              <DataTable.Title style={[styles.header, { width: 140 }]} ><Text style={styles.tableHeading}>Vendor ID</Text></DataTable.Title>
              <DataTable.Title style={[styles.header, { width: 140 }]} ><Text style={styles.tableHeading}>Invoice Date</Text></DataTable.Title>
            ...
            {items.slice(from, to).map((item, index) => (
              <DataTable.Row key={item.PurchaseOrderNumber}>
                ...
                <DataTable.Cell style={[styles.tablebody, { width: 150 }]}>{item.DiscountAmount}</DataTable.Cell>
                <DataTable.Cell style={[styles.tablebody, { width: 140 }]}>{item.VendorID}</DataTable.Cell>
                <DataTable.Cell style={[styles.tablebody, { width: 140 }]}>{moment(item.InvoiceDate).isValid() ? moment(item.InvoiceDate).format('DD/MM/YYYY') : ''}</DataTable.Cell>
                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}><Menu>
                  ...
                  <MenuOptions optionsContainerStyle={{ width: 'auto', padding: 10 }}>
                    <MenuOption onSelect={() => navigation.navigate(`Viewgoodreceipt`, { PurchaseOrderNumber: item.PurchaseOrderNumber })}>
                      <View style={styles.actions}>
                        <Text style={styles.actionstitle}>View</Text>
                        <AntDesign name="eye" size={20} color="#0A2DAA" />
                      </View>
                    </MenuOption>
                    <MenuOption onSelect={() => navigation.navigate(`GoodReceiptUpdate`, { PurchaseOrderNumber: item.PurchaseOrderNumber, myFunction: getapi })}>
                      <View style={styles.actions}>
        ...
        <View style={styles.buttonsection} >
          ...
          <Button radius={"md"} type="outline" containerStyle={{
            ...
            onPress={() => navigation.navigate('GoodReceiptCreate', { myFunction: getapi })}
          >
