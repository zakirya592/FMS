...
export default function Purchaserequesttable() {
  ...
  const handleCheckboxChange = (PurchaseRequestNumber) => {
    const updatedItems = items.map((item) =>
      item.PurchaseRequestNumber === PurchaseRequestNumber ? { ...item, selected: !item.selected } : item
    );
    setItems(updatedItems);
    // Update selectedItems state
    const selectedIds = updatedItems.filter((item) => item.selected).map((item) => item.PurchaseRequestNumber);
  ...
  const toggleDialog2 = (PurchaseRequestNumber) => {
    setVisible2(!visible2);
    setDeleteItemCode(PurchaseRequestNumber)
  };
  ...
  const updatbutton = () => {
    if (selectedItems.length >= 1) {
      navigation.navigate(`PurchaseRequestUpdate`, { PurchaseRequestNumber: selectedItems, myFunction: getapi })
    }
    else {
    ...
    <ScrollView>
      <View>
        ...
        <ScrollView horizontal vertical>
          <DataTable style={[styles.item, {
            ...
            <DataTable.Header>
              ...
              <DataTable.Title style={[styles.header, { width: 200 }]} ><Text style={styles.tableHeading}>Request By EmployeeID</Text></DataTable.Title>
              <DataTable.Title style={[styles.header, { width: 150 }]}><Text style={styles.tableHeading}>Verified By Employee</Text></DataTable.Title>
            ...
            {items.slice(from, to).map((item, index) => (
              <DataTable.Row key={item.PurchaseRequestNumber}>
                <DataTable.Cell style={[styles.tablebody, { width: 50 }]} >
                  <Checkbox
                    status={item.selected ? 'checked' : 'unchecked'}
                    onPress={() => handleCheckboxChange(item.PurchaseRequestNumber)}
                  />
                </DataTable.Cell>
                ...
                <DataTable.Cell style={[styles.tablebody, { width: 200 }]}>{item.RequestByEmployeeID}</DataTable.Cell>
                <DataTable.Cell style={[styles.tablebody, { width: 150 }]}>{item.VerifiedByEmpl}</DataTable.Cell>
                ...
                <DataTable.Cell style={[styles.tablebody, { width: 140, borderRightWidth: 1, }]}><Menu>
                  ...
                  <MenuOptions optionsContainerStyle={{ width: 'auto', padding: 10 }}>
                    <MenuOption onSelect={() => navigation.navigate(`Viewpurachaserequest`, { PurchaseRequestNumber: item.PurchaseRequestNumber })}>
                      <View style={styles.actions}>
                        <Text style={styles.actionstitle}>View</Text>
                        <AntDesign name="eye" size={20} color="#0A2DAA" />
                      </View>
                    </MenuOption>
                    <MenuOption onSelect={() => navigation.navigate(`PurchaseRequestUpdate`, { PurchaseRequestNumber: item.PurchaseRequestNumber, myFunction: getapi })}>
                      <View style={styles.actions}>
                        <Text style={styles.actionstitle}>Update</Text>
                        <FontAwesome5 name="pencil-alt" size={13} color="#0A2DAA" />
                      </View>
                    </MenuOption>
                    <MenuOption onSelect={() => toggleDialog2(item.PurchaseRequestNumber)}>
                      <View style={styles.actions}>
        ...
        <Dialog isVisible={visible2} onBackdropPress={toggleDialog2}>
          ...
          <Text>{`You want to delete this ${deleteItemCode} Purchase Request`}</Text>
          <Dialog.Actions >
        ...
        <AwesomeAlert
          ...
          message={`Purchase Request${deleteItemCode} has been deleted`}
